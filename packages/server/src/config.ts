...
//
// The host stays free to commit the registry anyway (see the ROADMAP's CI item), and the
// opt-out is to negate the pattern inside this file rather than to delete it: an existing
// file is never rewritten, but a deleted one comes back the next time a command creates
// the directory, since nothing else records that the host meant it to be gone.
const DATA_DIR_GITIGNORE = [
	"# Generated by Yosegi. Everything in this directory is build output.",
	'# Yosegi never rewrites this file. To commit the registry, add "!registry.json" below',
	"# — deleting the file instead only lasts until the next command recreates it.",
	"*",
	"",
].join("\n");
...
export async function ensureDataDir(dataDir = DEFAULT_DATA_DIR): Promise<void> {
	...
	// "wx" rather than a read-then-write: the host's copy is the one that matters, and an
	// existence check would still lose an edit written between the check and the write.
	try {
		await writeFile(join(dataDir, ".gitignore"), DATA_DIR_GITIGNORE, {
			flag: "wx",
		});
	} catch (error) {
