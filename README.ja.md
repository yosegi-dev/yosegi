...
- **正しくあることを安くする。** 丁寧なエージェントなら、同じ props をソースから 1 ファイルずつ掘り当てられます。
  Registry が変えるのはそのコストです。
  ベンチマーク済み: ソースを読んだ場合と同じ画面を、どの運び手よりも少ない読解量で出します。
  デザインシステム規模ではソースの 5 分の 1、パッケージ同梱の `.d.ts` の 3 分の 1 になります（[`docs/ja/benchmark.md`](./docs/ja/benchmark.md)）。
  当て推量は、正しくあることが高くつくときにエージェントがすることです。
- **書くつもりだった props 以外の選択肢も見える。** ソースを読む行為は、エージェントがすでに書くつもりでいた props の裏取りです。`component inspect` はそのコンポーネントが受け取れるものの全体（すべての variant、すべての enum の選択肢）を列挙します。
  存在を知らなかった選択肢は回避策で代替され、たいていホストの規約に反します。
- **Figma を介さずイテレーションする。** レビューで見るものと実装で使うものが同じになります。
  新しいビジュアル表現を決めるのは引き続き Figma の仕事です。

Registry とホスト自身の Story は、別々の問いに答えます。
コンポーネントが何を受け取るかは Registry が答えます。
既存の画面がそれをどう使っているか（作法や慣習）はホスト自身のコードだけが答え、Skill は両方を読みます。
