...
- **ホストをコンテキストに載せない。** ベンチマーク済み: Registry はソースを読んだ場合と同じ画面
  を、どの運び手より少ない読解量で出す。デザインシステム規模でソースの 5 分の 1、パッケージ同梱の
  `.d.ts` の 3 分の 1（[`docs/ja/benchmark.md`](./docs/ja/benchmark.md)）。
