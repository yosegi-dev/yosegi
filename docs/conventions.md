...
- is laid out one sentence per line after translating. The break goes after 「。」, except where the
  next character is ASCII — a break there renders as a space.
...
character width, Japanese one sentence each (tables, code blocks, and front matter are exempt from
both):
