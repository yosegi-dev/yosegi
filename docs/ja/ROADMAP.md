...
成果物は `.stories.tsx` 1 本のままで、いまは画面状態ごとに 1 つの export を運ぶ。ベースの Story に
`variants` のエントリごとの export が加わり、どの状態も同じツリーとその差分から描画される。これで
ファイルの形は決まった。1 つの Story モジュールに複数 export が既定であり、状態ごとに 1 ファイルで
はない。

残る半分は 2 つ目の emit ターゲットである。実装へ渡すことを考えると、合成コンポーネント
（`screen.tsx`）とそれを呼ぶ薄い Story に分ける形が考えられる。エミッタはすでに JSX 描画と CSF
文書の組み立てに分かれている（`render.ts` / `csf.ts`）ため、コンポーネントターゲットは描画側を
再利用し、自分の文書の形だけを持てばよい。
...
- Screen JSON 側に境界の表現が入るまでは Story ファイル 1 本を既定とする。
