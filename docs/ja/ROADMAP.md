...
`example apply` はあわせて、テンプレートの import 一覧と、インラインのモックデータがどこにあるかも報告します。
次に編集すべき場所がそこだからです。
...
その一般化がこの項目であり、PoC は実装済みです。
そのホストのテンプレート 11 件のカタログに対して `list` を、うち 3 件（1 件は数百行規模）に `apply` を実行し、複製したファイルはいずれも無編集でホストの型チェックと lint を通りました。
