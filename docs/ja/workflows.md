...
新しいビジュアル表現を決めるのは引き続き Figma の仕事であり、Figma 自身の MCP も Code Connect を通じて既存のコードへ踏み込んでいます。
どちらか一方を選ぶ話ではありません。
Yosegi が扱うのは、いま手元にあるコンポーネントで組める画面を、JSX が 1 行も無い段階で Registry と突き合わせて見せるところまでです。
